...
function HandlerGang:onEnter(seat_index, operReqData, provide_player)
	...
	if operReqData.weave_kind == const.Action.PENG_GANG then 
		--删除提供牌玩家的弃牌区最後一張
	...
	elseif operReqData.weave_kind == const.Action.ZHI_GANG then 
		--刪除手牌 一张
	...
	elseif operReqData.weave_kind == const.Action.AN_GANG then
		--刪除手牌
		local del_card = operReqData.center_card
	...
	if operReqData.weave_kind == const.Action.ZHI_GANG or 
		operReqData.weave_kind == const.Action.PENG_GANG then 
		
		hasAction = self:_checkOthersAction({const.Action.JIE_PAO}, operReqData.center_card)
	end 
...
function HandlerGang:_onOperateCardReq(msg_id, uid, data)
	...
	if data.weave_kind == const.Action.JIE_PAO then 
